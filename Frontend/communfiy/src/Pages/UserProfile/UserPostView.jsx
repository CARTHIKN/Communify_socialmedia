...
function UserPostView() {
  ...
  useEffect(() => {
    const checkUserLikesPost = async () => {
      try {
        const response = await axios.get(baseUrl + '/api/home/user/check-user-likes/', {
          params: {
            username: username, 
            post_id: postId,
          },
  ...
  useEffect(() => {
    const fetchLikeAndCommentCounts = async () => {
      try {
        const response = await axios.get(baseUrl + '/api/home/user/like-comments-count/', {
          params: {
            post_id: postId,
          },
        });
  ...
  const handleLike = async () => {
    try {
        const res = await axios.post(baseUrl + '/api/home/user/post/like/', { postId: postId, username:username },);
  ...
  const handleComment = async () => {
    try {
      const mainCommentsResponse = await axios.get(baseUrl + `/api/home/user/comment/${postId}/`);
      ...
      const fetchRepliedComments = async () => {
        const promises = mainCommentsData.map(async (mainComment) => {
          const repliedCommentsResponse = await axios.get(baseUrl+ `/api/home/user/comment/replied-comments/?parent_comment_id=${mainComment._id}`);
